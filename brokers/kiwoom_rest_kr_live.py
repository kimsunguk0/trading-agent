"""Kiwoom REST KR live adapter for production credentials."""
...
class KiwoomRestKrLiveAdapter:
    """Live Kiwoom adapter with the live credential naming convention."""

    name: str = "kiwoom_rest_kr_live"
    capabilities: BrokerCapabilities = KIWOOM_KR_LIVE_CAPS
    ...
    def __init__(
        ...
        self.app_key = app_key or os.getenv("KIWOOM_LIVE_APP_KEY") or os.getenv("KIWOOM_APP_KEY", "")
        self.app_secret = app_secret or os.getenv("KIWOOM_LIVE_APP_SECRET") or os.getenv("KIWOOM_APP_SECRET", "")
        if not self.app_key or not self.app_secret:
            raise ValueError("APP_KEY/APP_SECRET are required")
        self.account_no = account_no or os.getenv("KIWOOM_LIVE_ACCOUNT_NO", "")
    ...
    async def get_positions(self, account_id: str) -> dict[str, Decimal]:
        # Kiwoom API position endpoint is not fully modeled in this skeleton.
