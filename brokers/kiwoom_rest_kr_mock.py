"""Kiwoom REST KR mock adapter with rate limiting and websocket stream support."""
...
class KiwoomApiError(RuntimeError):
    """Raised when Kiwoom API returns non-zero rt_cd."""

    def __init__(self, rt_cd: str, message: str, payload: dict[str, Any] | None = None) -> None:
        super().__init__(f"Kiwoom API error {rt_cd}: {message}")
        self.rt_cd = rt_cd
...
class KiwoomRestKrMockAdapter:
    name: str = "kiwoom_rest_kr_mock"
    capabilities: BrokerCapabilities = KIWOOM_KR_MOCK_CAPS
    ...
    def __init__(
        ...
        self.app_key = app_key or os.getenv("APP_KEY") or os.getenv("KIWOOM_APP_KEY", "")
        self.app_secret = app_secret or os.getenv("APP_SECRET") or os.getenv("KIWOOM_APP_SECRET", "")
    ...
    async def _request_access_token(self) -> None:
        ...
        # expire 10min before API deadline
        self._access_token_expires_at = self._now() + timedelta(seconds=max(expires_in, 600))
    ...
    async def get_order_status(self, order_intent_id: str) -> OrderAck | None:
        ...
        payload = await self._request(
            ...
            tr_id="VTTC8001R",
            params={"CANO": self.account_no, "ACNT_PRDT_CD": "01", "INQR_STRT_DT": self._now().strftime("%Y%m%d"), "INQR_END_DT": self._now().strftime("%Y%m%d")},
        )
        ...
        for row in rows:
            ...
            if "CANCEL" in status_text:
                ...
                status = OrderStatus.PARTIALLY_FILLED if filled < _to_decimal(row.get("ORD_QTY", "0")) else OrderStatus.FILLED
            return OrderAck(
                order_id=broker_order_id,
                ...
                average_fill_price=_to_decimal(row.get("AVG_PRC") or row.get("price") or "0"),
                fills=[
                    Fill(
                        order_id=broker_order_id,
                        order_intent_id=order_intent_id,
                        quantity=filled,
                        price=_to_decimal(row.get("AVG_PRC") or row.get("price") or "0"),
                    )
                ],
            )
        return None
    ...
    def get_account(self, account_id: str) -> Account:
        # compatibility helper; detailed positions are resolved from broker balance endpoint in runtime
        return Account(account_id=account_id, cash_balance=Decimal("0"))


# runtime protocol compatibility aliases
