# 주식 자동매매 멀티 에이전트 시스템 — 전체 설계서 (v3.3)
...
- **§22 신설 — 전략 카탈로그**: 5개 실전 전략 YAML 추가
  ...
  - `candle_probability_v1` (캔들 패턴별 과거 승률 기반)
  - `analyst_report_momentum_v1` (증권사 리포트 목표가 상향 추종)
  - `us_to_kr_followthrough_v1` (전일 미국 테마 강세 → 다음날 한국 동조)
  - `event_anticipation_v1` (법안·FDA·실적 등 이벤트 D-7 선매수)
- **신규 Collector 2개**: Report Collector, Legislative/Event Calendar Collector
- **신규 Agent 3개**: Theme/Sector Agent, Report Analyst Agent, Catalyst Calendar Agent
...
| | Report Collector ★ | 없음 | 불가 | 1일 (장전) |
| | Event Calendar Collector ★ | 없음 | 불가 | 1일 |
...
| | Report Analyst Agent ★ | 높음 | 불가 | 1일 (장전) |
| | Catalyst Calendar Agent ★ | 높음 | 불가 | 1일 |
| **컨텍스트** | Market Regime Agent | 낮음 | 불가 | 5분 |
| | Theme/Sector Agent ★ | 낮음 | 불가 | 1일 |
...
│   │   ├── macro_calendar.py
│   │   ├── corporate_action.py
│   │   ├── report_collector.py      # ★ v3.3 증권사 리포트
│   │   └── event_calendar.py        # ★ v3.3 법안·FDA·실적 일정
│   ├── analysts/
│   │   ├── news_analyst.py
│   │   ├── catalyst_hunter.py
│   │   ├── bear_case.py
...
│   │   ├── fundamental.py
│   │   ├── verification.py
│   │   ├── report_analyst.py        # ★ v3.3
│   │   └── catalyst_calendar.py     # ★ v3.3
│   ├── context/
│   │   ├── market_regime.py          # ★
│   │   ├── theme_sector.py           # ★ v3.3 테마 집계
...
│   │   ├── volume_spike_no_move_v1.yaml      # ★ v3.3 (스킬 #3)
│   │   ├── candle_probability_v1.yaml        # ★ v3.3 (스킬 #5)
│   │   ├── analyst_report_momentum_v1.yaml   # ★ v3.3 (스킬 #6)
│   │   ├── us_to_kr_followthrough_v1.yaml    # ★ v3.3 (스킬 #2)
│   │   └── event_anticipation_v1.yaml        # ★ v3.3 (스킬 #4)
│   ├── universe/                       # ★ v3.3
│   │   ├── us_themes.yaml               # 미국 테마별 티커 (스킬 #1)
│   │   └── theme_us_kr_map.yaml         # 미–한 테마 매핑 (스킬 #2)
│   ├── risk/
│   │   ├── paper.yaml
│   │   ├── live_approval.yaml
│   │   └── live_auto.yaml
...
- 단순 룰 전략 YAML: `kr_opening_range_breakout` + **`volume_spike_no_move_v1`** (§22.3, 스킬 #3 — 순수 코드라 시스템 검증에 최적)
...
- 일일·주간 손실 한도 강제
- Report Collector + Report Analyst Agent + **`analyst_report_momentum_v1`** (§22.5, 스킬 #6 — Collector+LLM 첫 통합)
...
### MVP 5 (3~4주) — 미국주식 + RAG + 테마 추종

**목표**: 멀티마켓 + 학습 루프 + 미–한 동조 전략

- `KisOverseasAdapter` (US)
- 환율 처리, US 캘린더
- 미국 전략 (`us_earnings_reaction_v1`)
- Theme/Sector Agent + 유니버스 정의 (§22.2, 스킬 #1)
- **`us_to_kr_followthrough_v1`** (§22.6, 스킬 #2 — 미–한 동조율 백테스트 검증 후)
...
### MVP 6+ — 토스 API + 이벤트 선매수 + 고도화

- `TossInvestAdapter` (API 출시 시)
- Event Calendar Collector + Catalyst Calendar Agent + **`event_anticipation_v1`** (§22.7, 스킬 #4 — 가장 복잡, 이벤트 인프라 필요)
- 전략 카탈로그 확장
...
실제 매매 경험에서 나온 6개 스킬을 시스템 구성요소로 변환한 것. 각 전략은 YAML로 정의되고, 필요한 Collector·Agent를 명시한다.

### 22.0 스킬 → 시스템 매핑 요약

| # | 매매 스킬 | 시스템 매핑 | LLM | 난이도 | 구현 우선순위 |
|---|---|---|---|---|---|
| 1 | 미국 섹터/테마 묶기 | 유니버스 정의 + Theme/Sector Agent | 거의 X | 낮음 | 인프라 (전략 전 선행) |
| 2 | 다음날 한국주식 따라사기 | `us_to_kr_followthrough_v1` | 일부 | 중간 | 4순위 |
| 3 | 거래량 10배 + 변동성 없음 | `volume_spike_no_move_v1` | X | 낮음 | **1순위** |
| 4 | 법안·이슈 D-7 선매수 | `event_anticipation_v1` + Catalyst Calendar Agent | O | 높음 | 5순위 |
| 5 | 캔들 확률 매매 | `candle_probability_v1` | X | 중간 | 2순위 |
| 6 | 아침 증권사 리포트 정리 | `analyst_report_momentum_v1` + Report Agent | O | 중간 | 3순위 |

### 22.1 신규 컴포넌트

이 전략들을 위해 추가되는 Collector·Agent·Config:

**Collectors**:
- `Report Collector` — 네이버 금융 리서치, 한경컨센서스, 증권사 리포트 수집. 목표주가·투자의견 변경 파싱.
- `Event Calendar Collector` — 법안 표결 일정(의회), FDA PDUFA, 실적 캘린더, 지수 리밸런싱, 락업 해제일 수집.

**Agents**:
- `Theme/Sector Agent` — 미국 테마별 등락·자금흐름·거래량 집계. "오늘 어느 테마가 강했나" 산출.
- `Report Analyst Agent` (LLM) — 리포트에서 강한 시그널 추출, 복수 증권사 동시 상향 가중.
- `Catalyst Calendar Agent` (LLM) — 이벤트가 어느 종목에 호재인지·영향도·확실성 분석, 캘린더 등록·일정 변경 감지.

**Configs**:
- `configs/universe/us_themes.yaml` — 미국 테마별 티커
- `configs/universe/theme_us_kr_map.yaml` — 미국 테마 ↔ 한국 종목 매핑

### 22.2 유니버스 정의 (스킬 #1)

전략이 아니라 **모든 테마 전략의 기반 데이터**.

```yaml
# configs/universe/us_themes.yaml
themes:
  space_aerospace:   { name_kr: 우주항공,   tickers: [RKLB, LUNR, ASTS, RDW] }
  semiconductors:    { name_kr: 반도체,     tickers: [NVDA, AMD, AVGO, TSM] }
  robotics:          { name_kr: 로봇,       tickers: [ISRG, ABB] }
  drones:            { name_kr: 드론,       tickers: [AVAV, KTOS] }
  quantum:           { name_kr: 양자컴퓨팅,  tickers: [IONQ, RGTI, QBTS] }
  power_semi:        { name_kr: 전력반도체,  tickers: [WOLF, ON] }
  nuclear:           { name_kr: 원전,       tickers: [SMR, OKLO, CCJ, LEU] }
# 티커는 예시. 실제 구성 종목은 주기적으로 갱신 필요.
```

```yaml
# configs/universe/theme_us_kr_map.yaml
theme_mapping:
  nuclear:
    us: [SMR, OKLO, CCJ]
    kr: [["두산에너빌리티", "034020"], ["한전기술", "052690"]]
  defense_drone:
    us: [KTOS, AVAV]
    kr: [["한화에어로스페이스", "012450"], ["LIG넥스원", "079550"]]
  semiconductors:
    us: [NVDA, AMD]
    kr: [["삼성전자", "005930"], ["SK하이닉스", "000660"], ["한미반도체", "042700"]]
# 매핑은 가설. 동조화 강도는 §22.8 백테스트로 검증 필요.
```

`Theme/Sector Agent`가 매일 이 유니버스 기준으로 테마별 성과를 집계한다.

### 22.3 전략: volume_spike_no_move (스킬 #3) — 1순위

거래량이 평소 10배인데 가격이 횡보 = 큰 손의 은밀한 매집 가능성. **순수 코드, LLM 불필요.** 가장 먼저 구현·검증.

```yaml
# configs/strategies/volume_spike_no_move_v1.yaml
strategy_id: volume_spike_no_move_v1
version: 1
market: KR
description: "거래량 급증 + 가격 횡보 = 은밀한 매집 포착"
...
entry:
  all_of:
    - volume.today >= avg_volume.20d * 10
    - abs(price.change_pct_today) <= 0.03        # 가격 변동 ±3% 이내
    - price.position_in_range >= 0.4             # 당일 고저 범위 하단 아님
    - price.not_already_run_up == true           # 직전 급등 후 아님
    - liquidity.value_ok == true
  optional_confirm:
    - orderbook.bid_strength > orderbook.ask_strength
    - no_negative_news == true                   # ★ 악재성 거래량 배제 (News Analyst 연계)

exit:
  stop_loss_pct: 0.04
  take_profit:
    - { pct: 0.07, qty_ratio: 0.5 }
    - { pct: 0.15, qty_ratio: 0.5 }
  time_stop_days: 5
...
**함정**: 거래량 10배가 악재로 인한 투매일 수 있음. `no_negative_news`로 걸러야 함. 이 한 가지만 News Analyst의 도움이 필요.

### 22.4 전략: candle_probability (스킬 #5) — 2순위

캔들 패턴별 과거 승률 기반. **순수 기술적 전략.** 단, 캔들 단독 승률은 거의 50%이므로 반드시 거래량·국면과 결합.

```yaml
# configs/strategies/candle_probability_v1.yaml
strategy_id: candle_probability_v1
version: 1
market: KR
description: "캔들 패턴별 과거 승률 + 거래량·국면 결합 확률 매매"

patterns: [hammer, bullish_engulfing, morning_star, three_white_soldiers]

entry:
  all_of:
    - candle.pattern_detected in patterns
    - candle.pattern.historical_win_rate >= 0.6   # 과거 승률 임계
    - candle.pattern.sample_size >= 30            # 표본 30+ (과적합 방지)
    - volume.confirms_pattern == true
    - regime.favorable == true
    - price.near_support == true                  # 지지선 근처
...
**함정**: 캔들 패턴 단독은 동전 던지기. `historical_win_rate`와 `sample_size`를 백테스트로 실측해서 임계값을 정해야 함. 막연한 "망치형이면 산다"는 금물.

### 22.5 전략: analyst_report_momentum (스킬 #6) — 3순위

증권사 리포트 목표가 상향·매수 전환 종목 추종. Report Collector + Report Analyst Agent 필요.

```yaml
# configs/strategies/analyst_report_momentum_v1.yaml
strategy_id: analyst_report_momentum_v1
version: 1
market: KR
description: "증권사 리포트 목표가 상향/매수 전환 추종"

entry:
  all_of:
    - report.target_price_change == "raised"
    - report.target_upside_pct >= 0.20
    - report.rating_action in ["buy_initiation", "upgrade"]
    - report.broker_count >= 2                    # 2개 이상 증권사 동시
    - news.no_prior_run_up == true                # ★ 이미 선반영 안 됨
...
variant:
  pullback_entry:                                 # 변형: 발표 직후 추격 대신 눌림목
    wait_for: "report_date + 1~3d"
    entry_on: "first_pullback_to_support"
```

**함정**: 리포트는 발표 시점에 이미 주가 반영된 경우 많음. `no_prior_run_up` 필수. 발표 직후 갭상승 추격은 물림. 눌림목 변형 권장.

### 22.6 전략: us_to_kr_followthrough (스킬 #2) — 4순위

전일 미국 테마 강세 → 다음날 한국 동조 종목 따라매수. 미국 데이터 수집 + 테마 매핑 필요.

```yaml
# configs/strategies/us_to_kr_followthrough_v1.yaml
strategy_id: us_to_kr_followthrough_v1
version: 1
market: KR
description: "전일 미국 테마 강세 → 다음날 한국 동조 종목 시초가 따라매수"

trigger:
  schedule: "before_kr_open"
  condition:
    - us_theme.overnight_return >= 0.05           # 미국 테마 평균 +5%
    - us_theme.breadth >= 0.6                     # 테마 종목 60%+ 상승 (테마 전체 강세)
    - us_theme.is_theme_wide == true              # ★ 개별 이슈 아님 (News Analyst 판단)

entry:
  all_of:
    - kr_symbol.theme == us_theme.name
    - kr_symbol.gap_up_pct <= 0.04                # 시초가 갭 4% 이하 (추격 금지)
    - kr_symbol.liquidity_ok == true
  timing: "market_open + 5min"                    # 시초가 변동성 후

exit:
  stop_loss_pct: 0.03
  take_profit_pct: 0.05
  time_stop: "same_day_close"                     # 당일 청산 (오버나잇 회피)
...
**함정**: 미국-한국 동조화는 테마·시기에 따라 들쭉날쭉. News Analyst가 "테마 전체 강세 vs 개별 종목 이슈"를 구분해야 함(개별 이슈는 동조 안 됨). §22.8 백테스트로 테마별 동조율 먼저 검증.

### 22.7 전략: event_anticipation (스킬 #4) — 5순위

법안·FDA·실적 등 예정된 호재 D-7 선매수, 이벤트 직전 청산. "소문에 사서 뉴스에 판다." Event Calendar Collector + Catalyst Calendar Agent 필요. 가장 복잡.

```yaml
# configs/strategies/event_anticipation_v1.yaml
strategy_id: event_anticipation_v1
version: 1
market: [KR, US]
description: "예정된 호재 이벤트 D-7~D-1 선매수, 이벤트 직전/직후 청산"

event_sources:
  - legislative_calendar        # 법안 표결 (예: 클래리티 법안)
  - earnings_calendar
  - fda_pdufa_dates             # 신약 승인 예정일
  - product_launch
  - index_rebalancing           # 지수 편입/편출
  - unlock_dates                # 락업 해제 (악재로 역이용 가능)

entry:
  timing: "event_date - 7d"     # D-7부터 분할 진입
  all_of:
    - event.confidence >= 0.7    # 이벤트 발생 확실성
    - event.expected_impact == "positive"
    - symbol.not_already_priced_in == true

exit:
  primary: "event_date - 1d"     # 이벤트 직전 청산
  alt: "event_date + 0d if momentum_strong"

risk:
  max_position_pct_nav: 0.02
  max_concurrent_positions: 4
  scale_in: [0.4, 0.3, 0.3]      # D-7, D-5, D-3 분할
```

**함정**: 법안 등은 일정·통과 여부가 계속 변동. `event.confidence`를 LLM이 뉴스 모니터링하며 갱신, 일정 연기 시 대응 로직 필요. 이벤트 당일 보유는 "뉴스에 팔아라" 원칙 위배 — 기본은 직전 청산.

### 22.8 전략별 백테스트 우선 검증 항목

각 전략은 라이브 전에 다음을 **반드시 데이터로 확인**:

| 전략 | 검증 항목 |
|---|---|
| volume_spike_no_move | 매집 신호 후 N일 수익률 분포, 악재 거래량 오탐률 |
| candle_probability | 패턴별 실제 승률, 표본 수, 국면별 차이 |
| analyst_report_momentum | 발표 시점 선반영 정도, 눌림목 vs 추격 성과 차이 |
| us_to_kr_followthrough | **테마별 미–한 동조율** (가장 중요), 갭 크기별 성과 |
| event_anticipation | 이벤트 유형별 D-7 진입 수익률, 일정 연기 빈도 |

특히 `us_to_kr_followthrough`와 `candle_probability`는 가설 검증 성격이 강해서, 백테스트 결과가 나쁘면 전략을 폐기하거나 조건을 대폭 수정해야 한다.

### 22.9 구현 순서 (권장)

```
선행: Theme/Sector 유니버스 정의 (§22.2)
  ↓
1순위: volume_spike_no_move   — 순수 코드, 시스템 검증용
  ↓
2순위: candle_probability     — 백테스트로 진짜 승률 확인 연습
  ↓
3순위: analyst_report_momentum — Collector + LLM 첫 통합
  ↓
4순위: us_to_kr_followthrough  — 미국 데이터 + 테마 매핑
  ↓
5순위: event_anticipation     — 이벤트 캘린더 인프라 (가장 복잡)
```
